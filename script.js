...
// Configuración de CLABE - REEMPLAZA con tu CLABE interbancaria real
const CLABE_INTERBANCARIA = '638180000176182306';

// Inicialización
document.addEventListener('DOMContentLoaded', () => {
    // Leer parámetros de URL y reemplazar guiones con espacios
    ...
    // Si no hay parámetros y NO estamos en index.html, redirigir a index (setup)
if ((!fromName || !toName) && !window.location.pathname.includes('index.html')) {
...
function personalizeMessages() {
    ...
    // Mensajes según tipo: esposa (default), novios, amigos
    ...
    if (successMessage1 && successMessage2) {
        if (tipoValentine === 'esposa') {
            // Mensaje para esposos (default) - ¡Tus textos aquí!
            successMessage1.textContent = fromName ? `${fromName}: Me haces re felizzz.` : 'Me haces re felizzz.';
            successMessage2.textContent = `¡Te recontra amo, con amor: Anto! 💖`;
        } else if (tipoValentine === 'novios') {
            // Mensaje para novios - Cambié a tus textos también (si quieres diferentes, dime)
            successMessage1.textContent = fromName ? `${fromName}: Me haces re felizzz.` : 'Me haces re felizzz.';
            successMessage2.textContent = `¡Te recontra amo, con amor: Anto! 💖`;
        } else {
            // Mensaje para amigos - Cambié a tus textos también
            successMessage1.textContent = fromName ? `${fromName}: Me haces re felizzz.` : 'Me haces re felizzz.';
            successMessage2.textContent = `¡Te recontra amo!, con amor: Anto 💖`;
        }
    }
